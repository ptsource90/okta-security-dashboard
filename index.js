...
function clearTokens() {
  tokenStore = { accessToken: null, refreshToken: null, idToken: null, expiresAt: 0, oktaDomain: null, clientId: null };
  ["okta_refresh","okta_domain","okta_client_id","pkce_verifier","oauth_state","okta_domain_pending","okta_client_id_pending"]
    .forEach(k => sessionStorage.removeItem(k));
}

function isExpired() { return Date.now() >= tokenStore.expiresAt - 60_000; }
...
window.startLogin = async function () {
  const domain   = document.getElementById("okta-domain").value.trim().replace(/\/$/,"");
  const clientId = document.getElementById("okta-client-id").value.trim();
  if (!domain)   { showError("Please enter your Okta domain first."); return; }
  if (!clientId) { showError("Please enter your Client ID first."); return; }
  ...
  const params = new URLSearchParams({
    ...
    scope: CONFIG.scopes, redirect_uri: getRedirectUri(), state,
    code_challenge_method: "S256", code_challenge: challenge,
  });

  window.open(`https://${domain}/oauth2/v1/authorize?${params}`, "okta_auth", "width=520,height=660,left=200,top=100");
  setStatus("Waiting for authentication in popup…");
};

window.addEventListener("message", async (event) => {
  if (event.data?.type === "okta_callback") await handleCallback(event.data.url);
});
...
async function handleCallback(callbackUrl) {
  ...
  if (error) { showError(`Auth error: ${error} — ${url.searchParams.get("error_description") ?? ""}`); return; }
  if (!code) { showError("No code received in callback."); return; }
  if (retState !== sessionStorage.getItem("oauth_state")) { showError("State mismatch — possible CSRF."); return; }
  ...
  ["pkce_verifier","oauth_state","okta_domain_pending","okta_client_id_pending"].forEach(k => sessionStorage.removeItem(k));
  ...
  try {
    const resp = await fetch(`https://${domain}/oauth2/v1/token`, {
      ...
      body: new URLSearchParams({ grant_type: "authorization_code", client_id: clientId, redirect_uri: getRedirectUri(), code, code_verifier: verifier }),
    });
    if (!resp.ok) { const e = await resp.json().catch(()=>({})); throw new Error(e.error_description ?? resp.statusText); }
    storeTokens(await resp.json(), domain, clientId);
    onAuthenticated();
  } catch (err) { showError(`Token exchange failed: ${err.message}`); }
}

async function refreshAccessToken() {
  if (!tokenStore.oktaDomain || !tokenStore.refreshToken) throw new Error("No refresh token");
  const resp = await fetch(`https://${tokenStore.oktaDomain}/oauth2/v1/token`, {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: new URLSearchParams({ grant_type: "refresh_token", client_id: tokenStore.clientId, scope: CONFIG.scopes, refresh_token: tokenStore.refreshToken }),
  });
  if (!resp.ok) { clearTokens(); showLoginScreen(); throw new Error("Refresh token expired — please log in again."); }
...
async function fetchLogs(since) {
  ...
  const resp   = await fetch(`https://${tokenStore.oktaDomain}/api/v1/logs?${params}`, {
...
function getAiApiKey() { return sessionStorage.getItem("ai_api_key") || ""; }
...
window.sendAiMessage = async function () {
  ...
  if (!apiKey) { showError("Enter your Claude API key in the AI panel first."); return; }
  if (!currentLogs.length) { showError("No log data loaded yet — refresh the dashboard first."); return; }
...
window.clearAiChat = function () {
  ...
  appendAiMessage("assistant", "Chat cleared. Ask me anything about your Okta logs.");
};
...
function buildSystemPrompt() {
  ...
  const policy    = logs.filter(l => l.eventType?.startsWith("policy.lifecycle"));

  const evtCounts = {};
  logs.forEach(l => { evtCounts[l.eventType] = (evtCounts[l.eventType] ?? 0) + 1; });
  const topEvts = Object.entries(evtCounts).sort((a,b)=>b[1]-a[1]).slice(0,12)
    .map(([t,c]) => `  ${t}: ${c}`).join("\n");

  const failedByActor = {};
  failed.forEach(l => { const a = l.actor?.alternateId ?? "?"; failedByActor[a] = (failedByActor[a] ?? 0) + 1; });
  const topFailed = Object.entries(failedByActor).sort((a,b)=>b[1]-a[1]).slice(0,8)
    .map(([a,c]) => `  ${a}: ${c} failures`).join("\n");

  const failedByIp = {};
  failed.forEach(l => { const ip = l.securityContext?.ipAddress ?? "?"; failedByIp[ip] = (failedByIp[ip] ?? 0) + 1; });
  const topIps = Object.entries(failedByIp).sort((a,b)=>b[1]-a[1]).slice(0,8)
    .map(([ip,c]) => `  ${ip}: ${c} failures`).join("\n");

  const notable = logs
    .filter(l => l.outcome?.result === "FAILURE" || l.severity === "WARN" || l.severity === "ERROR")
    .sort((a,b) => new Date(b.published)-new Date(a.published))
    .slice(0, 40)
    .map(l => `  [${l.published}] ${l.eventType} | actor:${l.actor?.alternateId ?? "?"} | result:${l.outcome?.result} | ip:${l.securityContext?.ipAddress ?? "?"} | msg:${l.outcome?.reason ?? ""}`)
    .join("\n");

  const uniqueActors = [...new Set(logs.map(l => l.actor?.alternateId).filter(Boolean))];
...
window.onEventsSearch = function (e) { eventTableState.search = e.target.value; eventTableState.page = 1; renderEventsPanel(); };
window.onEventsSortChange = function (e) { eventTableState.sort = e.target.value; eventTableState.page = 1; renderEventsPanel(); };
window.onEventsPage = function (step) {
  const totalPages = Math.max(1, Math.ceil(applyEventFilters().length / EVENTS_PAGE_SIZE));
  eventTableState.page = Math.min(Math.max(1, eventTableState.page + step), totalPages);
  renderEventsPanel();
};
...
let nextRefreshAt = 0, countdownInterval = null;
function updateCountdown() {
  ...
  countdownInterval = setInterval(() => {
    ...
    if (el) el.textContent = `Auto-refresh ${Math.floor(secs/60)}:${String(secs%60).padStart(2,"0")}`;
  }, 1000);
}

window.manualRefresh = function () { loadDashboard(); scheduleAutoRefresh(); };

window.logout = function () {
  clearTokens(); clearInterval(refreshTimer); clearInterval(countdownInterval);
  showLoginScreen();
...
function renderDashboard(logs) {
  ...
  const policyChg = logs.filter(l => l.eventType?.startsWith("policy.lifecycle"));
  const suspicious= logs.filter(l => l.outcome?.result === "FAILURE" && l.eventType === "user.session.start");
  const uniqueIps = new Set(logs.map(l => l.securityContext?.ipAddress).filter(Boolean));
  ...
  const riskScore = Math.min(100, Math.round((failed.length / Math.max(logs.length, 1)) * 200 + suspicious.length * 4));
  const riskEl = document.getElementById("stat-risk");
  riskEl.textContent = riskScore;
  riskEl.parentElement.className = "stat-card " + (riskScore > 60 ? "red" : riskScore > 25 ? "yellow" : "green");

  // Severity
  const sev = { DEBUG: 0, INFO: 0, WARN: 0, ERROR: 0 };
  logs.forEach(l => { const s = l.severity?.toUpperCase(); if (s in sev) sev[s]++; });
  ["DEBUG","INFO","WARN","ERROR"].forEach(s => {
    document.getElementById(`sev-${s.toLowerCase()}`).style.width = pct(sev[s], logs.length);
    document.getElementById(`sev-${s.toLowerCase()}-lbl`).textContent = `${s} ${sev[s]}`;
  });

  renderHourlyChart(logs);

  // Suspicious logins
  renderTable("suspicious-table", suspicious.slice(0, 25),
  ...
  // Top actors
  const actors = {};
  logs.forEach(l => { const a = l.actor?.alternateId ?? "unknown"; actors[a] = (actors[a] ?? 0) + 1; });
  const topActors = Object.entries(actors).sort((a,b)=>b[1]-a[1]).slice(0,10);
  document.getElementById("top-actors").innerHTML = topActors.map(([name, count]) =>
    `<div class="actor-row">
  ...
  const evtCounts = {};
  logs.forEach(l => { evtCounts[l.eventType] = (evtCounts[l.eventType] ?? 0) + 1; });
  const topEvts = Object.entries(evtCounts).sort((a,b)=>b[1]-a[1]).slice(0,8);
  document.getElementById("event-types-list").innerHTML = topEvts.map(([t, c]) =>
    `<div class="actor-row">
  ...
  // Event log table
  eventTableState.rows = [...logs].sort((a,b) => new Date(b.published)-new Date(a.published)).slice(0, 200);
...
function renderHourlyChart(logs) {
  const hours = {};
  for (let i = 23; i >= 0; i--) hours[new Date(Date.now() - i * 3600_000).getHours()] = 0;
  logs.forEach(l => { const h = new Date(l.published).getHours(); hours[h] = (hours[h] ?? 0) + 1; });
  const vals = Object.values(hours);
  const max  = Math.max(...vals, 1);
  document.getElementById("hourly-chart").innerHTML = Object.keys(hours).map((h, i) =>
    `<div class="bar-col">
...
function renderTable(id, rows, headers, mapper) {
  ...
  if (!rows.length) { el.innerHTML = '<p class="empty">No events found.</p>'; return; }
  el.innerHTML = `<table><thead><tr>${headers.map(h=>`<th>${h}</th>`).join("")}</tr></thead>
    <tbody>${rows.map(r=>`<tr>${mapper(r).map(c=>`<td>${c}</td>`).join("")}</tr>`).join("")}</tbody></table>`;
}
...
function applyEventFilters() {
  ...
  return eventTableState.rows
    .filter(l => {
      ...
      return [l.displayMessage, l.eventType, l.actor?.alternateId, l.outcome?.result, l.securityContext?.ipAddress]
        .some(v => String(v ?? "").toLowerCase().includes(query));
    })
    .sort((a, b) => {
      const cmp = (x, y) => String(x ?? "").localeCompare(String(y ?? ""), undefined, { numeric: true, sensitivity: "base" });
      switch (eventTableState.sort) {
        case "published_asc":  return new Date(a.published) - new Date(b.published);
        case "published_desc": return new Date(b.published) - new Date(a.published);
        case "event_asc":      return cmp(a.displayMessage ?? a.eventType, b.displayMessage ?? b.eventType);
        case "event_desc":     return cmp(b.displayMessage ?? b.eventType, a.displayMessage ?? a.eventType);
        case "actor_asc":      return cmp(a.actor?.alternateId, b.actor?.alternateId);
        case "actor_desc":     return cmp(b.actor?.alternateId, a.actor?.alternateId);
        case "outcome_asc":    return cmp(a.outcome?.result, b.outcome?.result);
        case "outcome_desc":   return cmp(b.outcome?.result, a.outcome?.result);
        default: return 0;
      }
    });
}
...
function renderEventsPanel() {
  ...
  eventTableState.page = Math.min(Math.max(1, eventTableState.page), totalPages);
  const start = (eventTableState.page - 1) * EVENTS_PAGE_SIZE;

  renderTable("events-table", rows.slice(start, start + EVENTS_PAGE_SIZE),
...
function updateTokenLifetimeBar() {
  ...
  bar.className   = "token-bar-fill " + (pctLeft < 15 ? "danger" : pctLeft < 40 ? "warn" : "ok");
  const mins = Math.floor(remaining / 60_000);
  const secs = Math.floor((remaining % 60_000) / 1000);
  lbl.textContent = remaining > 0 ? `Token: ${mins}m ${secs}s` : "Token expired — refreshing…";
  if (remaining <= 0 && tokenStore.refreshToken) refreshAccessToken().catch(e => showError(e.message));
}
...
function badge(result) {
  ...
  const cls = result === "SUCCESS" ? "success" : result === "FAILURE" ? "failure" : "neutral";
...
(function handlePopupReturn() {
  ...
  if ((params.has("code") || params.has("error")) && window.opener) {
    window.opener.postMessage({ type: "okta_callback", url: window.location.href }, "*");
    window.close();
  }
})();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (tokenStore.refreshToken && !tokenStore.accessToken) {
    ...
    refreshAccessToken().then(onAuthenticated).catch(() => showLoginScreen());
